...
class WidgetRegistry:
    ...
    def __repr__(self) ->str:
        return '{' + ', '.join(f'{k!r}: {v!r}' for k, v in sorted(self.
            _registry.items())) + '}'
    ...
    def _add(self, widget: Widget) ->None:
        if widget.id in self._registry:
            raise KeyError(
                f'There is already a widget with the id {widget.id!r}')
...
class App:
    ...
    def __init__(self, formal_name: (str | None)=None, app_id: (str | None)
        =None, app_name: (str | None)=None, *, icon: (IconContentT | None)=
        None, author: (str | None)=None, version: (str | None)=None,
        home_page: (str | None)=None, description: (str | None)=None,
        startup: (AppStartupMethod | None)=None, document_types: (list[type
        [Document]] | None)=None, on_running: (OnRunningHandler | None)=
        None, on_exit: (OnExitHandler | None)=None):
        ...
        if icon is None:
            ...
            self.icon = icon
        self._documents = DocumentSet(self, types=[] if document_types is
            None else document_types)
        if on_running:
    ...
    def is_bundled(self) ->bool:
        ...
        return Path(sys.executable).stem not in {'python',
    ...
    def main_window(self, window: (MainWindow | str | None)) ->None:
        if window is None or window is App.BACKGROUND or isinstance(window,
            Window):
            if isinstance(window, Window) and not window.closable:
                raise ValueError(
                    'The window used as the main window must be closable.')
            ...
            raise ValueError(
                f"Don't know how to use {window!r} as a main window.")
    ...
    def _create_standard_commands(self):
        ...
        for cmd_id in [Command.ABOUT, Command.EXIT, Command.VISIT_HOMEPAGE]:
            self.commands.add(Command.standard(self, cmd_id))
        if self.documents.types:
            default_document_type = self.documents.types[0]
            command = Command.standard(self, Command.NEW, action=
                simple_handler(self.documents.new, default_document_type))
            if command:
                if len(self.documents.types) == 1:
                    self.commands.add(command)
                else:
                    for i, document_class in enumerate(self.documents.types):
                        command = Command.standard(self, Command.NEW,
                            action=simple_handler(self.documents.new,
                            document_class))
                        command.text = (command.text +
                            f' {document_class.description}')
                        if i > 0:
                            command.shortcut = None
                            command._id = (
                                f'{command.id}:{document_class.extensions[0]}')
                            command.order = command.order + i
                        self.commands.add(command)
            for cmd_id in [Command.OPEN, Command.SAVE, Command.SAVE_AS,
                Command.SAVE_ALL]:
                self.commands.add(Command.standard(self, cmd_id))
    ...
    def startup(self) ->None:
        ...
        if self._startup_method:
            ...
            if content is None:
                raise ValueError(
                    "Your app's startup method has not provided any content for your app's main window. Did you remember to return the main content container in your startup method?"
                    )
    ...
    def in_presentation_mode(self) ->bool:
        ...
        return any(window.state == WindowState.PRESENTATION for window in
            self.windows)

    def enter_presentation_mode(self, windows: (list[Window] | dict[Screen,
        Window])) ->None:
    ...
    def add_background_task(self, handler: BackgroundTask) ->None:
        ...
        warnings.warn(
            'App.add_background_task is deprecated. Use asyncio.create_task(), or set an App.on_running() handler'
...
class DocumentApp(App):
    ...
    def document_types(self) ->dict[str, type[Document]]:
        ...
        warnings.warn(
            ...
            DeprecationWarning, stacklevel=2)
        return {extension: doc_type for doc_type in self.documents.types for
            extension in doc_type.extensions}
