...
class Group:
    ...
    def __init__(self, text: str, *, parent: (Group | None)=None, section:
        int=0, order: int=0, id: (str | None)=None):
        """
    ...
    def __repr__(self) ->str:
        parent_string = (f' parent={self.parent} section={self.section}' if
            self.parent is not None else '')
...
class Command:
    ...
    def __init__(self, action: (ActionHandler | None), text: str, *,
        shortcut: (str | Key | None)=None, tooltip: (str | None)=None, icon:
        (IconContentT | None)=None, group: Group=Group.COMMANDS, section:
        int=0, order: int=0, enabled: bool=True, id: str=None):
        """
    ...
    def enabled(self, value: bool) ->None:
        self._enabled = value and getattr(self.action, '_raw', True
            ) is not None
    ...
    def __repr__(self) ->str:
        return (
            f'<Command text={self.text!r} group={self.group} section={self.section} order={self.order}>'
            )
...
class Separator:
    ...
    def __repr__(self) ->str:
        return (
            f'<Separator group={None if self.group is None else self.group.text}>'
            )
...
class CommandSet(MutableSet[Command], MutableMapping[str, Command]):
    ...
    def __init__(self, on_change: (CommandSetChangeHandler | None)=None,
        app: (App | None)=None):
        """
    ...
    def add(self, *commands: (Command | None)):
        ...
        self._commands.update({cmd.id: cmd for cmd in commands if cmd is not
            None})
        if self.on_change:
            self.on_change()
    ...
    def __setitem__(self, id: str, command: Command) ->Command:
        if id != command.id:
            raise ValueError(
                f"Command has id {command.id!r}; can't add as {id!r}")
        self.add(command)
