...
if TYPE_CHECKING:
    ...
    HandlerGeneratorReturnT: TypeAlias = Generator[Union[float, None],
        object, GeneratorReturnT]
    HandlerSyncT: TypeAlias = Callable[..., object]
    HandlerAsyncT: TypeAlias = Callable[..., Awaitable[object]]
    HandlerGeneratorT: TypeAlias = Callable[..., HandlerGeneratorReturnT[
        object]]
...
async def long_running_task(interface: object, generator:
    HandlerGeneratorReturnT[object], cleanup: (HandlerSyncT | None)) ->(object
     | None):
    ...
    **DEPRECATED** - use async co-routines instead of generators.
    """
    warnings.warn(
        'Use of generators for async handlers has been deprecated; convert the handler to an async co-routine that uses `asyncio.sleep()`.'
    ...
    try:
        ...
        if cleanup:
            try:
                ...
                print('Error in long running handler cleanup:', e, file=sys
                    .stderr)
                traceback.print_exc()
            else:
                pass
...
async def handler_with_cleanup(handler: HandlerAsyncT, cleanup: (
    HandlerSyncT | None), interface: object, *args: object, **kwargs: object
    ) ->(object | None):
    try:
...
def wrapped_handler(interface: object, handler: (HandlerT | NativeHandler |
    None), cleanup: (HandlerSyncT | None)=None) ->WrappedHandlerT:
    ...
    if handler:
        ...
        def _handler(*args: object, **kwargs: object) ->object:
            if asyncio.iscoroutinefunction(handler):
                return asyncio.ensure_future(handler_with_cleanup(handler,
                    cleanup, interface, *args, **kwargs))
            else:
                try:
                    ...
                    if inspect.isgenerator(result):
                        return asyncio.ensure_future(long_running_task(
                            interface, result, cleanup))
                    else:
                        try:
                            if cleanup:
                                cleanup(interface, result)
                            ...
                            print('Error in handler cleanup:', e, file=sys.
                                stderr)
                            traceback.print_exc()
                        else:
                            pass
...
class OnResultT(Protocol):
    ...
    def __call__(self, result: Any, exception: (Exception | None)=None
        ) ->object:
        ...
