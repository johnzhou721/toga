...
class Icon:
    ...
    def __init__(self, path: (str | Path), *, system: bool=False):
        """Create a new icon.
        ...
        try:
            ...
            self.system = system
            if self.system:
                resource_path = Path(self.factory.__file__
                    ).parent / 'resources'
            else:
            ...
            if self.factory.Icon.SIZES:
                ...
                for size in self.factory.Icon.SIZES:
                    try:
                        full_path[size] = self._full_path(size=size,
                ...
                full_path = self._full_path(size=None, extensions=self.
                    factory.Icon.EXTENSIONS, resource_path=resource_path)
            self._impl = self.factory.Icon(interface=self, path=full_path)
        except FileNotFoundError:
            if path is _APP_ICON:
                if toga.App.app.is_bundled:
                    try:
                        self._impl = self.factory.Icon(interface=self, path
                            =None)
                    except FileNotFoundError:
                        print(
    ...
    def _full_path(self, size: (str | None), extensions: Iterable[str],
            ...
            for extension in extensions:
                for filename in [
                    ...
                    if icon_path.exists():
                        return icon_path
        for extension in extensions:
            for filename in [f'{self.path.stem}-{platform}{extension}',
