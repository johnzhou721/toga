...
class ImageConverter(Protocol):
    ...
    @staticmethod
    def convert_to_format(data: BytesLikeT, image_class: type[ExternalImageT]
        ) ->ExternalImageT:
...
class Image:
    ...
    def __init__(self, src: ImageContentT=NOT_PROVIDED, *, path: object=
        NOT_PROVIDED, data: object=NOT_PROVIDED):
        """Create a new image.
        ...
        if path is not NOT_PROVIDED:
            src = path
            warn('Path argument is deprecated, use src instead.',
                DeprecationWarning, stacklevel=2)
        elif data is not NOT_PROVIDED:
            src = data
            warn('Data argument is deprecated, use src instead.',
                DeprecationWarning, stacklevel=2)
        ...
        if isinstance(src, (bytes, bytearray, memoryview)):
            ...
            if not self._path.is_file():
                raise FileNotFoundError(
                    f'Image file {self._path} does not exist')
