...
def _determine_counterclockwise(anticlockwise, counterclockwise):
    num_supplied = sum(value is not None for value in [anticlockwise,
        counterclockwise])
    if num_supplied == 0:
        return False
    if num_supplied == 1:
        if anticlockwise is not None:
            warn(
                "Parameter 'anticlockwise' is deprecated. Use 'counterclockwise' instead."
                , DeprecationWarning, stacklevel=3)
            return anticlockwise
        return counterclockwise
    raise TypeError(
        "Received both 'anticlockwise' and 'counterclockwise' arguments")
...
class Fill(DrawingObject):
    ...
    def __init__(self, color: str=BLACK, fill_rule: FillRule=FillRule.NONZERO):
        super().__init__()
        self.color = color
    ...
    def __repr__(self) ->str:
        return (
            f'{self.__class__.__name__}(color={self.color!r}, fill_rule={self.fill_rule})'
            )
...
class Stroke(DrawingObject):
    ...
    def __init__(self, color: (Color | str | None)=BLACK, line_width: float
    ...
    def __repr__(self) ->str:
        return (
            f'{self.__class__.__name__}(color={self.color!r}, line_width={self.line_width}, line_dash={self.line_dash!r})'
            )
...
class BezierCurveTo(DrawingObject):
    ...
    def __init__(self, cp1x: float, cp1y: float, cp2x: float, cp2y: float,
        x: float, y: float):
        self.cp1x = cp1x
        self.cp1y = cp1y
        self.cp2x = cp2x
    ...
    def __repr__(self) ->str:
        return (
            f'{self.__class__.__name__}(cp1x={self.cp1x}, cp1y={self.cp1y}, cp2x={self.cp2x}, cp2y={self.cp2y}, x={self.x}, y={self.y})'
            )

    def _draw(self, impl: Any, **kwargs: Any) ->None:
        impl.bezier_curve_to(self.cp1x, self.cp1y, self.cp2x, self.cp2y,
...
class QuadraticCurveTo(DrawingObject):
    ...
    def __repr__(self) ->str:
        return (
            f'{self.__class__.__name__}(cpx={self.cpx}, cpy={self.cpy}, x={self.x}, y={self.y})'
            )
...
class Arc(DrawingObject):
    ...
    def __init__(self, x: float, y: float, radius: float, startangle: float
        =0.0, endangle: float=2 * pi, counterclockwise: (bool | None)=None,
        anticlockwise: (bool | None)=None):
        counterclockwise = _determine_counterclockwise(anticlockwise,
            counterclockwise)
        self.x = x
        self.y = y
        self.radius = radius
    ...
    def __repr__(self) ->str:
        return (
            f'{self.__class__.__name__}(x={self.x}, y={self.y}, radius={self.radius}, startangle={self.startangle:.3f}, endangle={self.endangle:.3f}, counterclockwise={self.counterclockwise})'
            )

    def _draw(self, impl: Any, **kwargs: Any) ->None:
        impl.arc(self.x, self.y, self.radius, self.startangle, self.
            endangle, self.counterclockwise, **kwargs)


class Ellipse(DrawingObject):

    def __init__(self, x: float, y: float, radiusx: float, radiusy: float,
    ...
    def __repr__(self) ->str:
        return (
            f'{self.__class__.__name__}(x={self.x}, y={self.y}, radiusx={self.radiusx}, radiusy={self.radiusy}, rotation={self.rotation:.3f}, startangle={self.startangle:.3f}, endangle={self.endangle:.3f}, counterclockwise={self.counterclockwise})'
            )

    def _draw(self, impl: Any, **kwargs: Any) ->None:
        impl.ellipse(self.x, self.y, self.radiusx, self.radiusy, self.
            rotation, self.startangle, self.endangle, self.counterclockwise,
            **kwargs)
...
class Rect(DrawingObject):
    ...
    def __repr__(self) ->str:
        return (
            f'{self.__class__.__name__}(x={self.x}, y={self.y}, width={self.width}, height={self.height})'
            )
...
class WriteText(DrawingObject):
    ...
    def __init__(self, text: str, x: float=0.0, y: float=0.0, font: (Font |
    ...
    def __repr__(self) ->str:
        return (
            f'{self.__class__.__name__}(text={self.text!r}, x={self.x}, y={self.y}, font={self.font!r}, baseline={self.baseline}, line_height={self.line_height})'
            )

    def _draw(self, impl: Any, **kwargs: Any) ->None:
        impl.write_text(str(self.text), self.x, self.y, self.font._impl,
            self.baseline, self.line_height, **kwargs)
