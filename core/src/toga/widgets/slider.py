...
class Slider(Widget):
    ...
    def __init__(self, id: (str | None)=None, style: (StyleT | None)=None,
        value: (float | None)=None, min: float=0.0, max: float=1.0,
        tick_count: (int | None)=None, on_change: (toga.widgets.slider.
        OnChangeHandler | None)=None, on_press: (toga.widgets.slider.
        OnPressHandler | None)=None, on_release: (OnReleaseHandler | None)=
        None, enabled: bool=True, **kwargs):
    ...
    def tick_value(self, tick_value: (int | None)) ->None:
        if self.tick_count is None:
            if tick_value is not None:
                raise ValueError(
                    'cannot set tick value when tick count is None')
        else:
...
class IntSliderImpl(SliderImpl):
    ...
    def set_value(self, value: float) ->None:
        ...
        self.set_int_value(0 if span == 0 else round((value - self.min) /
            span * self.get_int_max()))
        self.value = value
    ...
    def on_change(self) ->None:
        span = self.max - self.min
        self.value = self.min + self.get_int_value() / self.get_int_max(
            ) * span
