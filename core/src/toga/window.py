...
class DialogResultHandler(Protocol[_DialogResultT]):
    ...
    def __call__(self, window: Window, result: _DialogResultT, **kwargs: Any
        ) ->object:
...
class Window:
    ...
    def __init__(self, id: (str | None)=None, title: (str | None)=None,
        position: (PositionT | None)=None, size: SizeT=(640, 480),
        resizable: bool=True, closable: bool=True, minimizable: bool=True,
        on_close: (OnCloseHandler | None)=None, on_gain_focus: (
        OnGainFocusHandler | None)=None, on_lose_focus: (OnLoseFocusHandler |
        None)=None, on_show: (OnShowHandler | None)=None, on_hide: (
        OnHideHandler | None)=None, content: (Widget | None)=None) ->None:
        """Create a new Window.
        ...
        self._impl = getattr(self.factory, self._WINDOW_CLASS)(interface=
            self, title=title if title else self._default_title, position=
            None if position is None else Position(*position), size=Size(*size)
            )
    ...
    def close(self) ->None:
        ...
        if self.app.main_window == self:
            ...
            if (len(self.app.windows) == 1 and self.app._impl.
                CLOSE_ON_LAST_WINDOW and self.app.loop.is_running()):
                self.app.request_exit()
                close_window = False
        if close_window:
            self._close()
    ...
    def show(self) ->None:
        ...
        """
        if not self.visible:
            if self.state in {WindowState.MINIMIZED, WindowState.FULLSCREEN,
                WindowState.PRESENTATION}:
                raise ValueError(
                    f'A window in {self.state} state cannot be shown.')
            else:
                self._impl.show()
    ...
    def position(self, position: PositionT) ->None:
        if self.state in {WindowState.FULLSCREEN, WindowState.PRESENTATION}:
            raise RuntimeError(
                f'Cannot change window position while in {self.state}')
        absolute_origin = self._app.screens[0].origin
    ...
    def screen(self, app_screen: Screen) ->None:
        ...
        self._impl.set_position(original_window_location - original_origin +
            new_origin)
    ...
    def hide(self) ->None:
        ...
        if self.visible:
            if self.state in {WindowState.MINIMIZED, WindowState.FULLSCREEN,
                WindowState.PRESENTATION}:
                raise ValueError(
                    f'A window in {self.state} state cannot be hidden.')
            else:
                self._impl.hide()
    ...
    def state(self, state: WindowState) ->None:
        ...
        elif not self.resizable and state in {WindowState.MAXIMIZED,
    ...
    async def dialog(self, dialog: dialogs.Dialog[dialogs.DialogResultT]
    ...
    def info_dialog(self, title: str, message: str, on_result: (
        DialogResultHandler[None] | None)=None) ->Dialog:
        """**DEPRECATED** - await :meth:`dialog` with an :any:`InfoDialog`"""
        warnings.warn(
            'info_dialog(...) has been deprecated; use dialog(toga.InfoDialog(...))'
            , DeprecationWarning, stacklevel=2)
        result = Dialog(self, on_result=wrapped_handler(self, on_result) if
            on_result else None)
        result.dialog = dialogs.InfoDialog(title, message)
        result.dialog._impl.show(self, result)
        return result

    def question_dialog(self, title: str, message: str, on_result: (
        DialogResultHandler[bool] | None)=None) ->Dialog:
    ...
    def confirm_dialog(self, title: str, message: str, on_result: (
        DialogResultHandler[bool] | None)=None) ->Dialog:
    ...
    def error_dialog(self, title: str, message: str, on_result: (
        DialogResultHandler[None] | None)=None) ->Dialog:
    ...
    def stack_trace_dialog(self, title: str, message: str, content: str,
        retry: bool=False, on_result: (DialogResultHandler[bool] |
        DialogResultHandler[None] | None)=None) ->Dialog:
        ...
        result = Dialog(self, on_result=wrapped_handler(self, on_result) if
            on_result else None)
        result.dialog = dialogs.StackTraceDialog(title, message=message,
            content=content, retry=retry)
        result.dialog._impl.show(self, result)
        return result

    def save_file_dialog(self, title: str, suggested_filename: (Path | str),
        file_types: (list[str] | None)=None, on_result: (
        DialogResultHandler[Path | None] | None)=None) ->Dialog:
        ...
        result = Dialog(self, on_result=wrapped_handler(self, on_result) if
            on_result else None)
        result.dialog = dialogs.SaveFileDialog(title, suggested_filename=
            suggested_filename, file_types=file_types)
        result.dialog._impl.show(self, result)
        return result

    def open_file_dialog(self, title: str, initial_directory: (Path | str |
        None)=None, file_types: (list[str] | None)=None, multiple_select:
        bool=False, on_result: (DialogResultHandler[list[Path]] |
        DialogResultHandler[Path] | DialogResultHandler[None] | None)=None
        ) ->Dialog:
        ...
        result = Dialog(self, on_result=wrapped_handler(self, on_result) if
            on_result else None)
        result.dialog = dialogs.OpenFileDialog(title, initial_directory=
            initial_directory, file_types=file_types, multiple_select=
            multiple_select)
        result.dialog._impl.show(self, result)
        return result

    def select_folder_dialog(self, title: str, initial_directory: (Path |
        str | None)=None, multiple_select: bool=False, on_result: (
        DialogResultHandler[list[Path]] | DialogResultHandler[Path] |
        DialogResultHandler[None] | None)=None) ->Dialog:
        ...
        result = Dialog(self, on_result=wrapped_handler(self, on_result) if
            on_result else None)
        result.dialog = dialogs.SelectFolderDialog(title, initial_directory
            =initial_directory, multiple_select=multiple_select)
        result.dialog._impl.show(self, result)
        return result
    ...
    def full_screen(self, is_full_screen: bool) ->None:
        warnings.warn(
            '`Window.full_screen` is deprecated. Use `Window.state` instead.',
            DeprecationWarning, stacklevel=2)
        target_state = (WindowState.FULLSCREEN if is_full_screen else
            WindowState.NORMAL)
